...
- **크라우딩 후속 스윕(07-15, 섹터 캡 4 기준선)으로 충돌 해소**: 캡4 +58.1%/MDD -15.2%, 캡5 +59.8%/-15.9%, 정의 강화(모멘텀25%/갭10%) +61.7%, 모멘텀 조건 단독 +57.1% — **전부 기준선(crowd3, +69.5%/MDD -10.9%)보다 나쁨**. "해제 +71.9%"는 섹터 캡 2 시절 측정값으로, 두 가드가 분산 역할을 분담하는 상호작용이 원인이었다. 섹터 캡 4 채택 후에는 크라우딩 3이 백테스트·라이브 반사실 양쪽에서 최선 — **크라우딩 조정 카드 종결, 현행 유지.**
