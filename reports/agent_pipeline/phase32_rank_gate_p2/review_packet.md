# Agent Review Packet - Run: phase32_rank_gate_p2
...
```
 ...
 48 files changed, 1497 insertions(+), 412 deletions(-)
```
...
index d6b0878..19bc493 100644
--- a/src/candidate_cache.py
+++ b/src/candidate_cache.py
@@ -37,6 +37,9 @@ from src.risk_manager import (
     get_recent_buy_symbols,
     get_today_buy_notional,
 )
+from src.position_dust import dust_position_min_usd, effective_position
...
@@ -354,6 +357,19 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
+        rank_ai_gate_scores = {}
     ...
     if getattr(settings, "use_ai_score", False):
@@ -443,6 +459,7 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
@@ -462,13 +479,21 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
                 market_clock=clock,
             )
...
+                    cash_buffer_mult=conviction.cash_buffer_mult,
                 ...
                 reason = risk.reason
@@ -478,13 +503,15 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
                     signal=signal,
                     cash=cash,
...
@@ -521,7 +548,23 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
@@ -529,6 +572,7 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
@@ -546,6 +590,7 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
@@ -573,6 +618,9 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
