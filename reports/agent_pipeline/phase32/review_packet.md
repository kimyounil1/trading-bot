# Agent Review Packet - Run: phase32
...
```
 ...
 scripts/run_paper_ops_bootstrap.sh                 |  29 +-
 src/benchmark_gap_report.py                        |  50 +++
 src/buy_guards.py                                  |   5 +-
 src/candidate_cache.py                             |  67 +++-
 ...
 48 files changed, 1580 insertions(+), 414 deletions(-)
```
...
+- [x] paper/dry-run **AI+LLM vs AI-only** buy path 리포트 — `src/paper_buy_validation_report.py`, `logs/paper_validation/latest_summary.json`
+- [ ] paper/dry-run 누적 해석 — agreement·skip 레이어 추세 2주+ 관찰 (bootstrap 반복)
+- [ ] `RUN_ID=phase32` pass complete (Codex scoped review) — `prompts/agy/phase32.md`
+
...
+- [x] **Label 후보 sweep** — 10d/20d/40d portfolio gate 기준 비교 완료, 전 후보 실패·champion 유지
...
+- [ ] **Rank AI paper 누적 검증 (2주)** — `rank_gate_paper_tracker` in paper_validation; dry-run 누적 후 `gate_ready=true`
...
+- [x] threshold retune 재실행 + 10d/40d label sweep (`label_challenger_sweep --force-retrain`) — 전 후보 portfolio gate 실패, champion 유지
+- [x] Codex dust P2: dust 시 `open_symbols` 제거 + cache `meaningful_positions_count` (main/candidate_cache)
+
+### 32-B LLM SDK (done)
...
+    )
     ...
     st.write(
@@ -542,6 +609,79 @@ def render_overview() -> None:
         },
     )
...
@@ -2398,81 +2601,107 @@ def render_ops_dashboard() -> None:
         )
...
@@ -354,6 +361,19 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
+        rank_ai_gate_scores = {}
     ...
     if getattr(settings, "use_ai_score", False):
@@ -410,6 +430,11 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
         except Exception as exc:
...
@@ -443,7 +468,6 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
@@ -462,13 +486,21 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
                 market_clock=clock,
             )
...
+                    cash_buffer_mult=conviction.cash_buffer_mult,
                 ...
                 reason = risk.reason
@@ -477,14 +509,16 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
@@ -521,7 +555,23 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
@@ -529,6 +579,7 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
@@ -546,6 +597,7 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
@@ -573,6 +625,9 @@ def build_candidate_cache() -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFr
...
index 8acf541..580996e 100644
--- a/src/paper_ops_summary.py
+++ b/src/paper_ops_summary.py
@@ -15,7 +15,10 @@ DEFAULT_AUDIT_PATH = Path("logs/execution_audit.csv")
...
@@ -35,10 +38,16 @@ def build_paper_ops_summary(
...
+    crowding_live = _load_json_if_exists(crowding_live_path) or {}
     ...
     llm_cache_keys = 0
@@ -54,6 +63,9 @@ def build_paper_ops_summary(
...
@@ -74,6 +86,26 @@ def build_paper_ops_summary(
...
@@ -82,10 +114,44 @@ def build_paper_ops_summary(
