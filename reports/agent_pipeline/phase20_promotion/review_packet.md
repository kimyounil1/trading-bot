# Agent Review Packet - Run: phase20_promotion
...
@@ -25,7 +27,11 @@ from src.ml_model import (
 )
...
@@ -332,6 +338,114 @@ def _score_threshold_row(row: dict) -> tuple[float, float, float, float]:
     )
...
+    model_wrapper,
+) -> dict:
+    """Six-month OOS portfolio backtest for model promotion (same window as threshold retune)."""
+    benchmark_df = (
...
+    if settings.use_ai_score:
+        print("Running 6-month OOS portfolio evaluation for promotion gates...")
