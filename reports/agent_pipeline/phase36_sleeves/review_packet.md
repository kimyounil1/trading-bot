# Agent Review Packet - Run: phase36_sleeves

## Implementation Agent
cursor

## Task Description
Phase 36 portfolio sleeves and alpha tournament track

## Changed Files
- TODO.md
- app/streamlit_app.py
- config/execution_lock.json
- config/schema/trading_config.schema.json
- docs/ai_authority_gates.md
...
- src/cms_helpers.py
- src/execution_audit_io.py
- src/live_readiness.py
- src/logger.py
- src/main.py
- src/order_intent.py
- src/settings.py
- src/trading_config_guard.py

## Git Diff Summary
```
 TODO.md                                     | 274 +++++++++++++++++++++-
 app/streamlit_app.py                        |  67 ++++++
 config/execution_lock.json                  |   4 +-
 config/schema/trading_config.schema.json    |   4 +-
 docs/ai_authority_gates.md                  |  20 ++
 ...
 scripts/run_paper_ops_bootstrap.sh          |   4 +
 src/cms_helpers.py                          |  48 ++++
 src/execution_audit_io.py                   |  12 +
 src/live_readiness.py                       |  13 ++
 src/logger.py                               |  12 +
 src/main.py                                 | 106 ++++++++-
 src/order_intent.py                         |  47 ++++
 src/settings.py                             |  16 ++
 src/trading_config_guard.py                 |  36 +++
 20 files changed, 1029 insertions(+), 149 deletions(-)
```
...
======================== 45 passed, 1 warning in 3.17s =========================
```

## Git Patch
```diff
diff --git a/TODO.md b/TODO.md
index d4e9ee2..dbc335d 100644
--- a/TODO.md
+++ b/TODO.md
@@ -26,7 +26,7 @@
...
@@ -2501,6 +2504,70 @@ def render_operator_decisions_panel() -> None:
                 )
...
diff --git a/config/schema/trading_config.schema.json b/config/schema/trading_config.schema.json
index c76086b..d927e56 100644
--- a/config/schema/trading_config.schema.json
+++ b/config/schema/trading_config.schema.json
@@ -26,6 +26,8 @@
       "type": "array",
       "items": { "type": "string" },
       "minItems": 1
-    }
+    },
+    "portfolio_sleeves_enabled": { "type": "boolean" },
+    "sleeves": { "type": "object" }
   }
 }
diff --git a/docs/ai_authority_gates.md b/docs/ai_authority_gates.md
index 79f9724..f673690 100644
--- a/docs/ai_authority_gates.md
+++ b/docs/ai_authority_gates.md
...
+        )
+    return rows
diff --git a/src/execution_audit_io.py b/src/execution_audit_io.py
index 639b398..18e155b 100644
--- a/src/execution_audit_io.py
+++ b/src/execution_audit_io.py
...
diff --git a/src/live_readiness.py b/src/live_readiness.py
index b2dd37d..f199b2a 100644
--- a/src/live_readiness.py
+++ b/src/live_readiness.py
...
@@ -114,6 +115,18 @@ def build_live_readiness_report(
...
index 1a3e5c1..8d5c251 100644
--- a/src/main.py
+++ b/src/main.py
@@ -62,7 +62,14 @@ from src.position_sizing import (
...
+                    order_amount = 0.0
+                else:
+                    core_budget_before = sleeve_allocator.order_budget_for(CORE_SLEEVE_ID)
+                    capped_amount = cap_order_amount_for_sleeve(
+                        order_amount,
+                        sleeve_id=CORE_SLEEVE_ID,
+                        allocator=sleeve_allocator,
+                    )
+                    if risk_allowed and capped_amount <= 0:
+                        risk_allowed = False
+                        risk_reason = (
+                            f"core sleeve budget exhausted (${core_budget_before:.2f})"
+                        )
+                        order_amount = 0.0
+                    elif risk_allowed and capped_amount < order_amount:
+                        order_amount = capped_amount
+
             log_signal(
                 ticker=ticker,
                 signal=signal,
@@ -1360,6 +1430,7 @@ def main() -> None:
...
+                max(
+                    0.0,
+                    sleeve_allocator.order_budget_for(CORE_SLEEVE_ID) - order_amount,
+                )
+                if sleeves_enabled(settings)
+                else None
+            ),
...
diff --git a/src/order_intent.py b/src/order_intent.py
index e80c65a..ac0ccc3 100644
--- a/src/order_intent.py
+++ b/src/order_intent.py
...
diff --git a/src/trading_config_guard.py b/src/trading_config_guard.py
index a71d47b..f0a3ebd 100644
--- a/src/trading_config_guard.py
+++ b/src/trading_config_guard.py
...
@@ -39,6 +44,16 @@ def load_profile_overlay(environment: str) -> dict[str, Any]:
...
+
 def apply_environment_profile(
     ...
     environment: str,
@@ -146,6 +161,27 @@ def validate_live_policies(
     else:
