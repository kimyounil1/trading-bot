# Agent Review Packet - Run: phase41_rank_primary_alignment
...
```
 ...
 config/profiles/tournament_paper.json              |   1 +
 config/strategy_config.json                        |  30 +-
 config/strategy_profiles.json                      |  29 +-
 ...
 tests/test_tournament_alpha_model.py               |  21 +
 57 files changed, 3477 insertions(+), 652 deletions(-)
```
...
+- 2026-07-14 채택 설정(Rank 상위 15%, Rank-only 토너먼트 정렬, 2배 상품 기준 손절 10%·익절 20%·트레일링 40%, 최대 30일, 부분익절 OFF)의 일봉 재생 결과는 선별 +28.52%, 별도 forward +13.66%, 2026-01-01~07-13 +53.26%입니다. 이는 체결 보장이 아닌 연구 결과입니다.
+
 ## 📂 프로젝트 구조
 
 ```text
@@ -149,6 +155,11 @@ bash scripts/run_sleeve_performance_report.sh                       # 슬리브
...
@@ -172,9 +183,9 @@ PR/push 시 GitHub Actions가 `requirements.txt` 설치 후 pytest·CMS import·
...
@@ -171,7 +171,7 @@
   ...
   "earnings_lookforward_days": 1,
-  "take_profit_partial_pct": 0.15,
+  "take_profit_partial_pct": 0.0,
...
+  "tournament_alpha_rank_weight": 1.0,
   ...
   "live_safety_enabled": false,
   "live_safety_kill_switch_path": "data/runtime/KILL_SWITCH",
diff --git a/config/strategy_profiles.json b/config/strategy_profiles.json
index cf21896..e11ade3 100644
...
+    include_external_filters: bool = False,
     ...
     return {
@@ -69,6 +82,21 @@ def portfolio_backtest_kwargs(
...
@@ -78,4 +106,29 @@ def portfolio_backtest_kwargs(
...
@@ -256,15 +288,62 @@ def _build_benchmark_relative_return_frame(
...
+    threshold_by_date: dict[pd.Timestamp, float] | None = None,
     ...
     benchmark_returns = _build_benchmark_relative_return_frame(
         benchmark_df,
@@ -275,13 +354,46 @@ def _apply_relative_strength_filter(
...
@@ -340,9 +452,16 @@ def run_portfolio_backtest(
...
@@ -352,6 +471,20 @@ def run_portfolio_backtest(
...
@@ -379,6 +512,18 @@ def run_portfolio_backtest(
...
@@ -405,30 +550,69 @@ def run_portfolio_backtest(
             macro_df=macro_df,
         )
...
@@ -453,6 +637,15 @@ def run_portfolio_backtest(
...
@@ -478,6 +671,56 @@ def run_portfolio_backtest(
...
@@ -488,6 +731,45 @@ def run_portfolio_backtest(
...
+            )
         if previous_trading_date is not None and cash < 0:
             ...
             margin_interest = (
@@ -505,16 +787,26 @@ def run_portfolio_backtest(
...
+            close = float(day_prices[ticker])
             position["highest_price"] = max(
                 ...
                 close,
@@ -545,8 +837,8 @@ def run_portfolio_backtest(
...
@@ -556,20 +848,72 @@ def run_portfolio_backtest(
...
+
             if exit_reason is not None:
                 ...
                 qty = position["qty"]
@@ -586,6 +930,8 @@ def run_portfolio_backtest(
                 trades.append(
                     {
                         "ticker": ticker,
+                        "signal_ticker": signal_ticker,
...
@@ -604,13 +950,33 @@ def run_portfolio_backtest(
         )
...
@@ -647,45 +1013,27 @@ def run_portfolio_backtest(
...
+                    )
                 if rank_buy_top_k_enabled(ops_settings):
                     max_select = max_rank_new_buys_per_run(
                         ops_settings,
@@ -727,7 +1075,7 @@ def run_portfolio_backtest(
...
@@ -739,14 +1087,14 @@ def run_portfolio_backtest(
...
@@ -755,8 +1103,33 @@ def run_portfolio_backtest(
...
+
                 ...
                 if close <= 0:
@@ -771,17 +1144,29 @@ def run_portfolio_backtest(
...
+                        held_signal_tickers,
                     ...
                     if not sector_ok:
@@ -792,7 +1177,7 @@ def run_portfolio_backtest(
...
@@ -801,7 +1186,7 @@ def run_portfolio_backtest(
                         continue
...
@@ -810,12 +1195,14 @@ def run_portfolio_backtest(
...
@@ -835,7 +1222,7 @@ def run_portfolio_backtest(
                 target_value = min(
                     target_value,
                     equity * adjust_position_cap_for_instrument(
-                        target_position_pct * leverage_factor,
...
@@ -866,13 +1253,21 @@ def run_portfolio_backtest(
...
@@ -882,16 +1277,28 @@ def run_portfolio_backtest(
                     continue
...
@@ -916,6 +1323,15 @@ def run_portfolio_backtest(
...
@@ -941,9 +1357,23 @@ def run_portfolio_backtest(
     else:
         win_rate = 0.0
...
@@ -998,3 +1428,11 @@ def save_portfolio_backtest_outputs(
...
@@ -34,6 +34,21 @@ def _rank_gate_enabled(settings: Any) -> bool:
...
@@ -189,6 +189,143 @@ def build_rank_ai_gate_scores(
     return scores
...
+    auto_discover_leveraged_products: bool = False
     ...
     max_leveraged_etf_positions: int = 1
     max_effective_leverage_exposure_pct: float = 1.25
@@ -170,6 +172,8 @@ class StrategySettings(StrategyProfile):
...
@@ -332,6 +336,8 @@ def validate_settings(settings: StrategySettings) -> StrategySettings:
...
@@ -377,6 +383,10 @@ def validate_settings(settings: StrategySettings) -> StrategySettings:
         raise ValueError(
...
@@ -16,7 +17,11 @@ from src.order_intent import build_buy_intent
...
@@ -38,6 +43,7 @@ from src.trading.bot_helpers import (
     order_is_filled,
 )
...
@@ -93,26 +99,38 @@ def run_buy_pipeline(ctx: TradingRunContext) -> None:
...
@@ -128,11 +146,11 @@ def run_buy_pipeline(ctx: TradingRunContext) -> None:
                     target_amount = 0.0
             else:
...
+                    ticker=execution_ticker,
                     ...
                     cash_buffer_mult=conviction.cash_buffer_mult,
                     settings=ctx.settings,
@@ -144,6 +162,7 @@ def run_buy_pipeline(ctx: TradingRunContext) -> None:
             if (
                 risk_allowed
...
@@ -167,6 +186,7 @@ def run_buy_pipeline(ctx: TradingRunContext) -> None:
...
@@ -190,6 +210,109 @@ def run_buy_pipeline(ctx: TradingRunContext) -> None:
...
@@ -217,7 +340,7 @@ def run_buy_pipeline(ctx: TradingRunContext) -> None:
...
@@ -230,7 +353,7 @@ def run_buy_pipeline(ctx: TradingRunContext) -> None:
...
+                    ticker=execution_ticker,
                     ...
                     cash=float(ctx.cash),
                     portfolio_value=float(ctx.account["portfolio_value"]),
@@ -246,7 +369,7 @@ def run_buy_pipeline(ctx: TradingRunContext) -> None:
...
@@ -276,7 +399,8 @@ def run_buy_pipeline(ctx: TradingRunContext) -> None:
             )
...
@@ -315,21 +439,24 @@ def run_buy_pipeline(ctx: TradingRunContext) -> None:
...
@@ -421,7 +548,7 @@ def run_buy_pipeline(ctx: TradingRunContext) -> None:
         )
         remaining_cap = max(
             0.0,
...
@@ -22,6 +23,7 @@ from src.trading.bot_helpers import (
     order_is_filled,
 )
 from src.trading.run_context import TradingRunContext
+from src.trading.leveraged_product_routing import resolve_leveraged_product_route
...
@@ -128,14 +130,40 @@ def run_tournament_buy_pipeline(ctx: TradingRunContext) -> None:
...
@@ -177,7 +205,10 @@ def run_tournament_buy_pipeline(ctx: TradingRunContext) -> None:
             continue
...
+        )
         ...
         if target_amount < 10.0:
             ctx.buy_summary_rows.append(
@@ -196,7 +227,7 @@ def run_tournament_buy_pipeline(ctx: TradingRunContext) -> None:
             tournament_settings,
         )
         leverage_cap = apply_effective_leverage_exposure_limits(
-            ticker=ticker,
+            ticker=execution_ticker,
             ...
             positions_by_symbol=ctx.positions_by_symbol,
@@ -236,19 +267,16 @@ def run_tournament_buy_pipeline(ctx: TradingRunContext) -> None:
             )
             continue
...
@@ -258,7 +286,7 @@ def run_tournament_buy_pipeline(ctx: TradingRunContext) -> None:
...
+                ticker=execution_ticker,
                 ...
                 market_clock=ctx.market_clock,
@@ -269,7 +297,7 @@ def run_tournament_buy_pipeline(ctx: TradingRunContext) -> None:
...
+            ctx.guard_open_symbols.add(execution_ticker)
             ctx.sleeve_ctx.consume_submit_budget(
                 order_amount,
                 sleeve_id=TOURNAMENT_SLEEVE_ID,
@@ -277,13 +305,13 @@ def run_tournament_buy_pipeline(ctx: TradingRunContext) -> None:
             audit_log(
                 ctx.audit_ctx,
...
+                signal=entry_signal,
                 ...
                 order_id=submission.order_id,
                 notional=order_amount,
@@ -296,15 +324,19 @@ def run_tournament_buy_pipeline(ctx: TradingRunContext) -> None:
             )
...
+  "generated_at": "2026-07-14T06:50:32Z",
...
diff --git a/tests/test_rank_ai_gate.py b/tests/test_rank_ai_gate.py
index bc1ecab..15667ca 100644
--- a/tests/test_rank_ai_gate.py
+++ b/tests/test_rank_ai_gate.py
...
@@ -31,6 +32,17 @@ def test_rank_ai_gate_effective_cutoff_uses_top_bucket_and_score_quantile():
...
index 3adc31c..22e4d7b 100644
--- a/tests/test_settings.py
+++ b/tests/test_settings.py
@@ -158,17 +158,20 @@ class SettingsTest(unittest.TestCase):
...
+        self.assertFalse(loaded.conviction_sizing_enabled)
...
+        self.assertTrue(loaded.rank_ai_primary_selector_enabled)
+        self.assertEqual(loaded.tournament_alpha_rank_weight, 1.0)
...
@@ -50,6 +51,26 @@ class TournamentAlphaModelTest(unittest.TestCase):
         )
